...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	attackorder: {
		...
		onHit(target, source, move) {
			// Attack Order normally reveals its category via animation on cart, but doesn't play either custom animation against allies
	...
	belch: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, sound: 1, bypasssub: 1, metronome: 1},
	...
	bleakwindstorm: {
		...
		onModifyMove(move, pokemon, target) {
			if (target && ['hail', 'snow', 'deltastream'].includes(target.effectiveWeather())) {
				move.accuracy = true;
			}
		},
		secondary: {
	...
	covet: {
		...
		pp: 25,
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1, noassist: 1, metronome: 1},
		onAfterHit(target, source, move) {
	...
	furycutter: {
		...
		basePowerCallback(pokemon, target, move) {
			...
			const bp = this.clampIntRange(move.basePower * pokemon.volatiles['furycutter'].multiplier, 1, 220);
			this.debug('BP: ' + bp);
			return bp;
		},
		category: "Physical",
	...
	gearup: {
		...
		onHitSide(side, source, move) {
			...
			for (const target of targets) {
				didSomething = this.boost({atk: 1, spa: 1, spe: 1}, target, source, move, false, true) || didSomething;
			}
			return didSomething;
		},
		secondary: null,
	...
	instruct: {
		...
		pp: 15,
		priority: 0,
		flags: {protect: 1, bypasssub: 1, allyanim: 1, failinstruct: 1, metronome: 1},
		onHit(target, source) {
	...
	overdrive: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, sound: 1, bypasssub: 1, pulse: 1, metronome: 1},
		secondary: null,
		target: "allAdjacentFoes",
		type: "Electric",
	},
	...
	petalblizzard: {
		...
		flags: {protect: 1, mirror: 1, metronome: 1, wind: 1},
		onModifyMove(move) {
			if (this.field.isWeather(['hail', 'snow'])) move.basePower = 135;
		},
		secondary: null,
		target: "allAdjacent",
		type: "Grass",
	...
	rage: {
		...
		basePower: 50,
		basePowerCallback(pokemon) {
			return Math.min(350, 50 + 30 * pokemon.timesAttacked);
		},
		category: "Physical",
		name: "Rage",
		pp: 20,
	...
	razorwind: {
		...
		pp: 10,
		priority: 0,
		flags: {charge: 1, protect: 1, mirror: 1, metronome: 1, nosleeptalk: 1, failinstruct: 1, wind: 1, slicing: 1},
		onTryMove(attacker, defender, move) {
			if (attacker.removeVolatile(move.id)) {
				return;
			}
	...
	rototiller: {
		...
		onHitField(target, source) {
			...
			for (const pokemon of targets) {
				this.boost({atk: 1, spa: 1, def: 1}, pokemon, source);
			}
		},
		secondary: null,
		target: "all",
	...
	sunsteelstrike: {
		...
		onBasePower(basePower, pokemon, target) {
			...
			const strongWeathers = ['sunnyday', 'desolateland'];
	...
	terastarstorm: {
		...
		pp: 5,
		priority: 0,
		flags: {protect: 1, mirror: 1, noassist: 1, failcopycat: 1, failmimic: 1, nosketch: 1, metronome: 1},
		onModifyType(move, pokemon) {
	...
	thousandwaves: {
		...
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, sound: 1, bypasssub: 1, nonsky: 1, metronome: 1},
		onHit(target, source, move) {
			if (source.isActive) target.addVolatile('trapped', source, move, 'trapper');
		},
		secondary: null,
